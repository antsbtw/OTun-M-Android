...
data class EncryptedSharePayload(
    ...
    companion object {
        ...
        private val json = Json {
            ...
            encodeDefaults = true
            isLenient = true  // 添加这行：允许宽松的JSON解析
        }
        ...
        fun fromUrlSafeString(urlSafeString: String): EncryptedSharePayload {
            ...
            return json.decodeFromString(jsonString)  // ← 使用配置的实例
        }

        // 也修改这个方法使用配置的实例
        fun toUrlSafeString(): String {
            val jsonString = json.encodeToString(this)  // ← 使用配置的实例
...
object ConfigurationCrypto {
    ...
    fun decryptConfig(payload: EncryptedSharePayload, password: String): CompactShareableConfig {
        ...
        try {
            ...
            Log.d(TAG, "Password bytes: ${password.toByteArray(Charsets.UTF_8).joinToString(",") { it.toString() }}")

            val saltBytes = Base64.decode(payload.salt, Base64.DEFAULT)
            val nonceBytes = Base64.decode(payload.nonce, Base64.DEFAULT)
            val encryptedBytes = Base64.decode(payload.encryptedData, Base64.DEFAULT)
            val authTagBytes = Base64.decode(payload.authTag, Base64.DEFAULT)

            Log.d(TAG, "Salt (hex): ${saltBytes.joinToString("") { "%02x".format(it) }}")
            Log.d(TAG, "Nonce (hex): ${nonceBytes.joinToString("") { "%02x".format(it) }}")
            Log.d(TAG, "EncryptedData length: ${encryptedBytes.size} bytes")
            Log.d(TAG, "AuthTag (hex): ${authTagBytes.joinToString("") { "%02x".format(it) }}")

            // 派生密钥并打印
            val key = deriveKey(password, saltBytes)
            Log.d(TAG, "Derived Key (hex): ${key.joinToString("") { "%02x".format(it) }}")
            Log.d(TAG, "===========================")
            ...
            val combined = encryptedBytes + authTagBytes
